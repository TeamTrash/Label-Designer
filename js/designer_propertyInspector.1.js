...
com.logicpartners.propertyInspector1 = function(designer, canvas) {
	this.canvas = canvas;
	...
	this.update = function(activeElement) {
		...
		var notEditable = activeElement == null ? [] : activeElement.readonly.concat(activeElement.hidden); 

		if (this.activeElement == activeElement) {
			for (var key in activeElement) {
				if (key != "readonly" && key != "hidden" && $.inArray(key, notEditable) == -1) {
			...
			for (var key in activeElement) {
				if (!keys[key] && $.inArray(key, activeElement.hidden) == -1) {
					keys[key] = true;
					
					if (key != "readonly" && key != "hidden" && getType.toString.call(activeElement[key]) != '[object Function]') {
